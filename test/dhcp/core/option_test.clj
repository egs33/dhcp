...
(deftest parse-options-test
  (testing "pad-option"
    (is (= [{:code 0, :type :pad, :length 1, :value []}]
           (option/parse-options
            (byte-array (concat magic-cookie [0]))))))
  (testing "end-option"
    (is (= [{:code 255, :type :end, :length 1, :value []}]
           (option/parse-options
            (byte-array (concat magic-cookie [255]))))))
  (testing "requested-ip-address"
    (is (= [{:code 50, :type :requested-ip-address, :length 6, :value [-64 -88 0 1]}]
           (option/parse-options
            (byte-array (concat magic-cookie [50 4 192 168 0 1]))))))
  (testing "multi options"
    (is (= [{:code 53, :type :dhcp-message-type, :length 3, :value [3]}
            {:code 61, :type :client-identifier, :length 8, :value [10 20 30 40 50 60]}
            {:code 50, :type :requested-ip-address, :length 6, :value [-64 -88 0 1]}
